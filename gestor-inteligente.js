(() => {
  ...
  function bracketView(list, grouped) {
    ...
    return `<div class="gi-bracket-scroll"><div class="gi-bracket">${[...columns]
      .sort(([a], [b]) => phaseOrder(a) - phaseOrder(b))
  ...
  function view() {
    ...
    return `<section id="giManager" data-tid="${escapeHtml(tournamentId)}">
      ...
      <div class="gi-metrics"><div><b>${list.length}</b><span>Jogos</span></div><div><b>${completed}</b><span>Finalizados</span></div><div><b>${goals}</b><span>Gols</span></div><div><b>${config.legs === 2 ? 'Ida e volta' : 'Jogo único'}</b><span>Mata-mata</span></div></div>
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .gi-empty{display:grid;place-items:center;gap:7px;min-height:210px;padding:22px;border:1px dashed var(--line-strong);border-radius:17px;color:var(--muted);text-align:center}.gi-bracket-scroll{overflow-x:auto}.gi-bracket{display:grid;grid-auto-flow:column;grid-auto-columns:220px;gap:11px;min-width:max-content}.gi-bracket section>h3{color:var(--gold-soft);font:900 18px "Barlow Condensed",sans-serif;text-transform:uppercase}.gi-bracket article{margin-bottom:8px;padding:9px;border:1px solid var(--line);border-radius:13px;background:linear-gradient(150deg,var(--surface-2),var(--surface));cursor:pointer}.gi-bracket article>div{display:grid;grid-template-columns:25px 1fr auto;align-items:center;gap:7px;padding:4px;border-radius:8px}.gi-bracket article>div.winner{color:var(--green);background:rgba(79,223,143,.06)}.gi-bracket article span{overflow:hidden;font-size:9px;text-overflow:ellipsis;white-space:nowrap}.gi-bracket article>small{display:block;color:var(--muted);font-size:7px;text-align:center}
