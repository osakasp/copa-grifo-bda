(() => {
  ...
  const phases = ['Quartas de final','Semifinal','Final'];
  let activePhase = 'Quartas de final';
  let frame = 0;
  ...
  function knockoutGames(){
    return games().filter(game => String(game?.id || '').startsWith('mata-') || /quartas|semifinal|semi-final|\bfinal\b/i.test(String(game?.phase || '')));
  }
  ...
  function groupComplete(){ return Boolean(window.ArenaBDASuperLeagueRule?.groupsComplete?.()); }
  function phasePanel(phase){
    ...
    if(phase === 'Quartas de final'){
      const ready = groupComplete();
      const button = ready && window.ArenaBDAAuth?.isAdmin?.() ? '<button type="button" class="primary arena-v4-generate-final">Gerar quartas</button>' : '';
      return `<div class="arena-v4-bracket-empty"><span>🏆</span><b>${ready?'Quartas prontas para gerar':'Aguardando fase de grupos'}</b><p>${ready?'Os 2 melhores de cada grupo formarão 4 quartas de final.':'Os 2 melhores de cada grupo avançam diretamente às quartas de final.'}</p>${button}</div>`;
    }
  ...
  function preferredPhase(){
    if(!structureExists()) return 'Quartas de final';
  ...
  document.addEventListener('click',event=>{
    ...
    if(phase){activePhase=phase.dataset.arenaV4Phase||'Quartas de final';render();return;}
  });
