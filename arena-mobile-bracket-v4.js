(() => {
  ...
  function knockoutGames(){
    const desired = Boolean(window.ArenaBDASuperLeagueRule?.finalStructureExists?.());
    return games().filter(game => {
      const phase = phaseFor(game);
      if(!phase) return false;
      if(desired) return true;
      return phase === 'Repescagem' || phase === 'Play-in';
    });
  }
  function phaseGames(phase){
    return knockoutGames().filter(game => phaseFor(game) === phase)
  ...
  function byId(reference){
    ...
    return knockoutGames().find(game => {
      const id = norm(game?.id).replace(/\s+/g,'');
  ...
  function done(game){
    if(['a','b'].includes(game?.wo)) return true;
    const a=score(game,'a'), b=score(game,'b');
    if(a == null || b == null) return false;
    return a !== b || (has(game.pa) && has(game.pb) && Number(game.pa) !== Number(game.pb));
  }
  ...
  function provisional(){ return window.ArenaBDASuperLeagueRule?.provisionalEntries?.() || []; }

  function waitingPanel(phase){
    const ready = groupsComplete();
    const count = provisional().filter(entry => entry.destination === 'Repescagem').length;
    const button = ready && window.ArenaBDAAuth?.isAdmin?.()
      ? '<button type="button" class="primary arena-v4-generate-final">Gerar fase final</button>' : '';
    if(phase === 'Repescagem'){
      return `<div class="arena-v4-bracket-empty"><span>🏆</span><b>${ready?'Repescagem pronta para gerar':`${count || 0} vaga(s) de 3º já confirmada(s)`}</b><p>${ready?'Os quatro 3º colocados formarão dois jogos de repescagem.':'Cada grupo encerrado confirma também o seu 3º colocado na repescagem.'}</p>${button}</div>`;
    }
    return `<div class="arena-v4-bracket-empty"><span>🏆</span><b>${esc(phase)} ainda não definida</b><p>A fase será preenchida automaticamente depois da etapa anterior.</p></div>`;
  }
  ...
  function render(){
    ...
    if(!shell){
      ...
      shell.className='arena-v4-bracket-shell';
      anchor.before(shell);
  ...
  function patchGenerator(){
    ...
    if(!button){
      ...
      button.type='button';
      button.className=`${legacy.className||'primary'} arena-v4-generate-final`;
      legacy.after(button);
    }
    const generated=structureExists();
    button.hidden=!window.ArenaBDAAuth?.isAdmin?.();
    button.disabled=!groupsComplete()||generated;
    const label=generated?'Fase final gerada':groupsComplete()?'Gerar fase final':'Aguardando fase de grupos';
  ...
  document.addEventListener('click',event=>{
    ...
    const generate=event.target.closest?.('.arena-v4-generate-final');
