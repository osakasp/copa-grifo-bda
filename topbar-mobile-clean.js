(() => {
  ...
  function decorateAdmin() {
    ...
    const validMarkup = Boolean(button.querySelector('.arena-clean-admin-icon'));
    if (button.dataset.cleanAdminState === state && validMarkup) return;
  ...
  style.textContent = `
    ...
    .arena-clean-panel-icon{font-size:15px;line-height:1}
    ...
    .arena-clean-admin-icon{font-size:15px;line-height:1}
    .arena-clean-admin-dot{position:absolute;right:5px;top:5px;width:7px;height:7px;border:2px solid #102018;border-radius:50%;background:var(--green);box-shadow:0 0 0 3px rgba(79,223,143,.12)}
    #cloudPanelBtn[hidden]{display:none!important}

    @media(max-width:720px){
      .topbar.arena-topbar-clean{
        position:sticky!important;top:6px!important;z-index:48!important;
        width:auto!important;min-height:62px!important;margin:6px 8px 12px!important;padding:7px 8px!important;
        display:flex!important;align-items:center!important;justify-content:space-between!important;gap:7px!important;
        border:1px solid rgba(242,215,125,.20)!important;border-radius:20px!important;
        background:linear-gradient(180deg,rgba(13,28,20,.96),rgba(5,13,9,.97))!important;
        box-shadow:0 13px 34px rgba(0,0,0,.34)!important;backdrop-filter:blur(20px)!important
      }
      .topbar.arena-topbar-clean .brand{
        width:46px!important;min-width:46px!important;flex:0 0 46px!important;margin:0!important;padding:0!important;
        display:block!important;border:0!important
      }
      .topbar.arena-topbar-clean .brand:before,.topbar.arena-topbar-clean .brand:after{display:none!important}
      .topbar.arena-topbar-clean .brand-copy{display:none!important}
      .topbar.arena-topbar-clean .brand-mark{
        width:46px!important;height:46px!important;margin:0!important;border-radius:14px!important;transform:none!important;
        box-shadow:0 8px 20px rgba(0,0,0,.24)!important
      }
      .topbar.arena-topbar-clean .top-actions{
        flex:1 1 auto!important;display:grid!important;
        grid-template-columns:40px 40px minmax(72px,92px) 42px!important;
        justify-content:end!important;align-items:center!important;gap:5px!important
      }
      .topbar.arena-topbar-clean #shareBtn{display:none!important}
      .topbar.arena-topbar-clean #arenaThemeToggle,
      .topbar.arena-topbar-clean #arenaNotificationsBtn{
      ...
      .topbar.arena-topbar-clean #cloudPanelBtn{
        width:100%!important;min-width:0!important;height:40px!important;min-height:40px!important;padding:0 9px!important;
        border-radius:12px!important;gap:5px!important
      }
      .topbar.arena-topbar-clean #cloudPanelBtn b{font-size:9px!important}
      .topbar.arena-topbar-clean #adminBtn{
        width:42px!important;min-width:42px!important;height:40px!important;min-height:40px!important;padding:0!important;
      ...
      html[data-theme="light"] .topbar.arena-topbar-clean{
        background:linear-gradient(180deg,rgba(250,252,250,.97),rgba(235,243,236,.97))!important;
        border-color:rgba(166,126,20,.24)!important;box-shadow:0 13px 32px rgba(39,72,49,.14)!important
      }
    ...
    @media(max-width:370px){
      ...
      .topbar.arena-topbar-clean .brand{width:42px!important;min-width:42px!important;flex-basis:42px!important}
      .topbar.arena-topbar-clean .brand-mark{width:42px!important;height:42px!important}
      .topbar.arena-topbar-clean .top-actions{grid-template-columns:38px 38px minmax(60px,78px) 38px!important;gap:4px!important}
      .topbar.arena-topbar-clean #arenaThemeToggle,
      .topbar.arena-topbar-clean #arenaNotificationsBtn,
      .topbar.arena-topbar-clean #cloudPanelBtn,
      .topbar.arena-topbar-clean #adminBtn{height:38px!important;min-height:38px!important}
      .topbar.arena-topbar-clean #arenaThemeToggle,
      .topbar.arena-topbar-clean #arenaNotificationsBtn{width:38px!important;min-width:38px!important}
      .topbar.arena-topbar-clean #adminBtn{width:38px!important;min-width:38px!important}
      .topbar.arena-topbar-clean #cloudPanelBtn{padding-inline:6px!important}
      .topbar.arena-topbar-clean #cloudPanelBtn b{font-size:8px!important}
    }
  `;
  ...
  window.ArenaBDATopbar = Object.freeze({ refresh });
  refresh();
})();