(() => {
  'use strict';
  if (window.ArenaBDASuperLeagueRule?.version >= 4) return;
  ...
  let frame=0, autoTimer=0, autoGenerating=false;
  ...
  function ensureConfig(){const a=tournaments(),i=a.findIndex(t=>String(t?.id||'')===TID);if(i<0)return false;const before=a[i],after=desiredTournament(before);const key=t=>[t?.qualifiersPerGroup,t?.groupSettings?.qualifiersPerGroup,t?.groupGenerator?.qualifiers,t?.groupGenerator?.knockoutMode,t?.groupGenerator?.knockoutGenerated,t?.groupGenerator?.knockoutGames,t?.groupGenerator?.autoKnockout,t?.groupGenerator?.autoKnockoutTrigger,t?.description];if(sig(key(before))===sig(key(after)))return false;const next=clone(a);next[i]=after;localStorage.setItem(TOURNAMENT_KEY,JSON.stringify(next));window.dispatchEvent(new CustomEvent('arena:tournaments-updated',{detail:{tournamentId:TID,reason:'super-league-auto-final-config'}}));return true}
  ...
  async function generate({automatic=false}={}){
    if(!isAdmin()){if(!automatic)notify('Apenas o administrador pode gerar a fase final');return false}
    ensureConfig();
    if(!groupsComplete()){if(!automatic)notify('Finalize todos os jogos da fase de grupos primeiro');return false}
    if(finalStructureExists()){if(!automatic)notify('A fase final completa já foi gerada');return true}
    const current=games(),ko=buildFinal(current);
    if(ko.length!==11){if(!automatic)notify('Não foi possível montar repescagem, play-in e mata-mata');return false}
    try{localStorage.setItem(BACKUP_KEY,JSON.stringify({savedAt:Date.now(),reason:automatic?'auto-generate-three-qualifier-final':'generate-three-qualifier-final',games:clone(current.filter(knockout))}))}catch{}
    const s=store();s[TID]=[...current.filter(g=>!knockout(g)),...ko];localStorage.setItem(MATCH_KEY,JSON.stringify(s));
    const a=tournaments(),i=a.findIndex(t=>String(t?.id||'')===TID),generatedAt=Date.now();
    if(i>=0){const t=desiredTournament(a[i]);t.status='Em andamento';t.phase='Repescagem';t.groupGenerator={...(t.groupGenerator||{}),qualifiers:3,knockoutGenerated:true,knockoutMode:'third-place-repechage-playin',repechageQualifiers:2,directQuarterfinalSeconds:2,playInQualifiers:2,knockoutGames:11,knockoutGeneratedAt:generatedAt,autoKnockout:true,autoKnockoutTrigger:'groups-complete'};a[i]=t;localStorage.setItem(TOURNAMENT_KEY,JSON.stringify(a))}
    const reason=automatic?'super-league-auto-final-generated':'super-league-three-qualifier-final-generated';
    window.dispatchEvent(new CustomEvent('arena:matches-updated',{detail:{tournamentId:TID,reason,count:ko.length,automatic}}));
    window.dispatchEvent(new CustomEvent('arena:tournaments-updated',{detail:{tournamentId:TID,reason,automatic}}));
    try{await window.ArenaBDASuperLeagueSyncGate?.uploadAdminLatest?.()}catch{}
    notify(automatic?'Fase de grupos encerrada • mata-mata criado automaticamente':'Fase final criada com repescagem, play-in, quartas, semifinal e final');
    refresh();
    return true
  }

  function scheduleAutoGeneration(reason='event',delay=550){
    if(autoTimer||autoGenerating||!isAdmin()||!groupsComplete()||finalStructureExists())return;
    const cloudState=document.documentElement.dataset.arenaSuperLeagueCloud||'idle';
    if(reason==='initial'&&cloudState!=='ready')return;
    if(reason==='cloud-sync'&&cloudState!=='ready')return;
    autoTimer=setTimeout(async()=>{
      autoTimer=0;
      if(autoGenerating||!isAdmin()||!groupsComplete()||finalStructureExists())return;
      if(document.documentElement.dataset.arenaSuperLeagueCloud==='loading'&&reason!=='score')return;
      autoGenerating=true;
      try{await generate({automatic:true})}finally{autoGenerating=false}
    },Math.max(150,Number(delay)||550))
  }
  ...
  document.addEventListener('click',e=>{const b=e.target.closest?.('#giManager[data-tid="bda-super-league"] [data-generate-knockout]');if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();generate({automatic:false})},true);
  ...
  window.addEventListener('arena:quick-score-saved',e=>{if(e.detail?.tournamentId===TID)scheduleAutoGeneration('score',650)});
  window.addEventListener('arena:matches-updated',e=>{if(e.detail?.tournamentId!==TID)return;const reason=String(e.detail?.reason||'');if(!reason.includes('final-generated'))scheduleAutoGeneration('matches',750)});
  window.addEventListener('arena:super-league-cloud-synced',e=>{if(e.detail?.tournamentId===TID&&e.detail?.state==='ready')scheduleAutoGeneration('cloud-sync',250)});
  window.addEventListener('arena:auth-changed',()=>{if(document.documentElement.dataset.arenaSuperLeagueCloud==='ready')scheduleAutoGeneration('auth',350)});
  new MutationObserver(refresh).observe(document.documentElement,{childList:true,subtree:true});
  window.ArenaBDASuperLeagueRule=Object.freeze({version:4,qualifiers:3,autoKnockout:true,groupsComplete,finalStructureExists,slices,buildFinal,generate,scheduleAutoGeneration,provisionalEntries,ensureTournamentConfig:ensureConfig,refresh});
  installStorageGuard();cleanupDirectSkeleton();ensureConfig();refresh();setTimeout(()=>scheduleAutoGeneration('initial',300),0);
})();