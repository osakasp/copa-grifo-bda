(() => {
  ...
  const itemBytes = value => new Blob([JSON.stringify(value)]).size;

  function validateDataset(name, values) {
    if (!Array.isArray(values)) throw new Error(`Dados inválidos em ${name}`);
    values.forEach((value, index) => {
      if (itemBytes(value) > MAX_ITEM_BYTES) throw new Error(`O item ${index + 1} de ${name} está grande demais para a nuvem`);
    });
  }
  ...
  function buildInterface() {
    ...
    if (adminPanel && !document.getElementById('cloudUploadBtn')) {
      ...
      tools.innerHTML = '<button class="primary" id="cloudUploadBtn" type="button">Enviar este aparelho para a nuvem</button><button class="ghost" id="cloudDownloadBtn" type="button">Baixar dados da nuvem</button>';
    ...
    uploadButton?.addEventListener('click', async () => {
      const message = cloudInitialized ? 'Substituir os dados da nuvem pelos dados deste aparelho?' : 'Usar os dados deste aparelho como primeira versão da nuvem?';
  ...
  async function writeDataset(name, values, previousCount, revision) {
    ...
    for (let index = values.length; index < previousCount; index += 1) operations.push({ type: 'delete', ref: root.doc(documentId(config.prefix, index)) });
    await runBatches(operations);
    knownCounts[name] = values.length;
  }
  async function readRemoteDataset(name, count, revision = '') {
    if (!count) return [];
    const snapshot = await root.where('dataset', '==', name).get();
    return snapshot.docs
      .map(document => document.data())
      .filter(data => !revision || !data.revision || data.revision === revision)
      .sort((a, b) => Number(a.position) - Number(b.position))
      .slice(0, count)
      .map(data => data.value);
  }
  function syncRuntime(name, values) {
    try {
      if (name === 'teams' && typeof teams !== 'undefined') teams = clone(values);
      if (name === 'champions' && typeof champions !== 'undefined') champions = clone(values);
    } catch (error) {
      console.warn(`Não foi possível atualizar ${name} em memória`, error);
    }
  }
  function refreshViews(names) {
    if (names.includes('teams') && typeof renderTeams === 'function') renderTeams();
    if (names.includes('champions') && typeof renderChampions === 'function') renderChampions();
    window.dispatchEvent(new CustomEvent('arena:cloud-data-updated', { detail: { names: [...names], source: 'firestore' } }));
  }
  function applyRemoteDatasets(remote) {
    ...
    try {
      Object.entries(remote).forEach(([name, values]) => {
        ...
        syncRuntime(name, values);
        changedNames.push(name);
      });
    } finally {
      applyingRemote = false;
    }
    if (changedNames.length) refreshViews(changedNames);
    return changedNames;
  }
  function errorMessage(error) {
    ...
    if (code.includes('invalid-argument')) return 'Uma imagem ou registro está grande demais para a nuvem';
  ...
  async function uploadAll() {
    ...
    try {
      ...
      await metaRef.set({ initialized: true, schemaVersion: 4, counts, revisions, updatedAt: serverTimestamp(), updatedBy: authCore.currentEmail() }, { merge: true });
  ...
  async function downloadAll(forceNotice = false) {
    ...
    try {
      ...
      const entries = await Promise.all(names.map(async name => [name, await readRemoteDataset(name, Number(meta.counts?.[name] || 0), meta.revisions?.[name] || '')]));
      ...
      setStatus('Sincronizado', 'ok');
      if (forceNotice) notify(changed.length ? 'Dados atualizados pela nuvem' : 'Este aparelho já está atualizado');
    } catch (error) {
      console.error(error);
  ...
  metaRef.onSnapshot(async snapshot => {
    ...
    currentMeta = meta;
    cloudInitialized = true;
    Object.keys(DATASETS).forEach(name => { knownCounts[name] = Number(meta.counts?.[name] || 0); });
    updateControls();
    ...
    try {
      const entries = await Promise.all(changedNames.map(async name => [name, await readRemoteDataset(name, Number(meta.counts?.[name] || 0), meta.revisions?.[name] || '')]));
      const remote = Object.fromEntries(entries);
      changedNames.forEach(name => { seenRevisions[name] = meta.revisions?.[name] || ''; });
      applyRemoteDatasets(remote);
      setStatus('Sincronizado', 'ok');
    } catch (error) {
      console.error(error);
      setStatus('Erro na nuvem', 'error', errorMessage(error));
