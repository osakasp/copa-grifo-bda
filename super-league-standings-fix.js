(() => {
  ...
  const score = game => game?.wo === 'a'
    ? [3,0]
    : game?.wo === 'b'
      ? [0,3]
      : [Number(game?.a)||0, Number(game?.b)||0];

  const knockout = game => /quartas|semifinal|semi-final|\bfinal\b|mata-super-league|qf\d|sf\d/i
  ...
  */
  ...
  function table(group) {
    return `<section class="stand-group" data-canonical-group="${esc(group.name)}">
      <header>
        <div><span class="eyebrow">Tabela oficial</span><h3>${esc(group.name)}</h3></div>
        <span>2 classificam • ${group.rows.length} clubes</span>
      </header>
      <div class="stand-scroll">
        <table>
          <thead><tr><th>#</th><th>Clube</th><th>PTS</th><th>J</th><th>V</th><th>E</th><th>D</th><th>GP</th><th>GC</th><th>SG</th><th class="stand-rank-key" aria-hidden="true">ORDEM</th><th>APR</th><th>Últimos</th></tr></thead>
          <tbody>${group.rows.map((row,index)=>{
            const meta=teamMeta(row.name);
            return `<tr class="${index<2?'qualified':''}">
              ...
              <td class="stand-rank-key" aria-hidden="true">${rankKey(row)}</td>
  ...
  function content() {
    ...
    return `<div class="stand-head">
      <div>
        <span class="eyebrow">Atualização instantânea</span>
        <h2>Classificação oficial</h2>
        <div class="stand-rule">Ordem: pontos, vitórias, saldo de gols e gols marcados. O APR é apenas informativo.</div>
      </div>
      <button class="primary" id="standPhoto">📸 Foto da tabela</button>
    </div>${highlights(data)}<div id="standCapture">${data.map(table).join('')}</div>`;
  }
  ...
  function refresh() {
    ...
    frame=requestAnimationFrame(()=>{
      ...
      panel.querySelectorAll('.stand-scroll').forEach(scroller => { scroller.scrollLeft = 0; });
    });
  }
  ...
  function ensureStyles() {
    ...
    style.textContent=`
      ...
      #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-rank-key{display:none!important}
      #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-rule{margin-top:5px;color:var(--muted);font-size:9px;line-height:1.5}

      @media(max-width:720px){
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-scroll{position:relative;overflow-x:auto;overscroll-behavior-x:contain}
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group table{min-width:900px}
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group th:first-child,
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group td:first-child{
          position:sticky;left:0;z-index:5;width:42px;min-width:42px;max-width:42px;background:#09150e
        }
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group th:nth-child(2),
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group td:nth-child(2){
          position:sticky;left:42px;z-index:4;width:176px;min-width:176px;max-width:176px;background:#09150e;box-shadow:8px 0 16px rgba(0,0,0,.16)
        }
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group thead th:first-child,
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group thead th:nth-child(2){z-index:8;background:#0b2e1b}
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group tr.qualified td:first-child,
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-group tr.qualified td:nth-child(2){background:#0d1c13}
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-club{min-width:0!important;width:100%}
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-club>span:last-child{min-width:0;overflow:hidden}
        #giManager[data-tid="${SUPER_LEAGUE_ID}"] #autoStandings .stand-club b{display:block;max-width:116px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      }
    `;
  ...
  ['arena:quick-score-saved','arena:matches-updated','arena:bundle-loaded','arena:cloud-ready','arena:tournaments-updated']
    .forEach(type=>window.addEventListener(type,()=>{loadRepair();refresh()}));

  window.addEventListener('storage',event=>{
    if(event.key===MATCH_KEY||event.key===TEAM_KEY){loadRepair();refresh();}
  });
  ...
  window.ArenaBDASuperLeagueStandingsFix=Object.freeze({
    ...
    ranking:'points-wins-goal-difference-goals-for'
  });
