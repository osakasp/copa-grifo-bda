...
function ensure(){
  ...
  const signature=JSON.stringify([currentId,tournament()?.format,games(currentId),clubs().map(t=>[t.name,t.badge,t.master])]);
...
document.addEventListener('click',event=>{
  const stand=event.target.closest('[data-standings-tab]');if(stand){active=true;$$('#giManager>nav button').forEach(b=>b.classList.toggle('active',b===stand));ensure();const panel=$('#autoStandings');if(panel){panel.innerHTML=content();lastSignature=JSON.stringify([currentId,tournament()?.format,games(currentId),clubs().map(t=>[t.name,t.badge,t.master])]);bindPhoto()}return}
