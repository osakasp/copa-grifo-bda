...
document.addEventListener('click',event=>{
  const stand=event.target.closest('[data-standings-tab],[data-tab="standings"]');if(stand){active=true;$$('#giManager>nav button').forEach(b=>{const selected=b===stand||b.dataset.tab==='standings';b.classList.toggle('active',selected);b.setAttribute('aria-selected',selected?'true':'false')});ensure();const panel=$('#autoStandings');if(panel){panel.innerHTML=content();lastSignature=JSON.stringify([currentId,tournament()?.format,tournament()?.groupSettings,tournament()?.qualifiersPerGroup,games(currentId),clubs().map(t=>[t.name,t.badge,t.master])]);bindPhoto()}return}
...
window.ArenaDOMEvents.subscribe(()=>{clearTimeout(timer);timer=setTimeout(ensure,80)},{selector:'#giManager,.gi-content,.gi-game'});
ensure();
})();
