...
const TK='bda-v3-tournaments',MK='bda-v3-confrontos',TEAMK='bda-v2-teams',SUPER_LEAGUE_ID='bda-super-league';
...
function superLeagueOwned(){return currentId===SUPER_LEAGUE_ID&&Boolean(window.ArenaBDASuperLeagueRuntimeFix?.calculate)}
function setCell(cell,value){const text=String(value);if(cell&&cell.textContent!==text)cell.textContent=text}
function syncSuperLeagueStandings(){
  if(!superLeagueOwned())return;
  const manager=$('#giManager'),panel=$('#autoStandings'),runtime=window.ArenaBDASuperLeagueRuntimeFix;
  if(!manager||manager.dataset.tid!==SUPER_LEAGUE_ID||!panel||panel.hidden||!runtime)return;
  runtime.refresh?.();
  requestAnimationFrame(()=>{
    ...
    data.forEach(group=>{
      ...
      if(!tbody)return;
      const byTeam=new Map([...tbody.querySelectorAll('tr')].map(row=>[norm(row.querySelector('.stand-club b')?.textContent),row]));
      const ordered=group.rows.map(entry=>({entry,row:byTeam.get(norm(entry.name))})).filter(item=>item.row);
      ordered.forEach(({entry,row},index)=>{
        const current=tbody.children[index];
        if(current!==row)tbody.insertBefore(row,current||null);
        const cells=row.cells;
        setCell(row.querySelector('.stand-pos'),index+1);
        setCell(cells?.[2],entry.pts);setCell(cells?.[3],entry.j);setCell(cells?.[4],entry.v);setCell(cells?.[5],entry.e);setCell(cells?.[6],entry.d);setCell(cells?.[7],entry.gp);setCell(cells?.[8],entry.gc);
        setCell(cells?.[9],`${entry.sg>0?'+':''}${entry.sg}`);setCell(cells?.[10],`${efficiency(entry)}%`);
        cells?.[9]?.classList.toggle('positive',entry.sg>0);cells?.[9]?.classList.toggle('negative',entry.sg<0);
        row.classList.toggle('qualified',index<q);
      });
    });
  });
}
function ensure(){
  ...
  const nav=$(':scope>nav',manager);if(nav){let b=$('[data-standings-tab],[data-tab="standings"]',nav);if(!b){b=document.createElement('button');b.textContent='Classificação';nav.append(b)}b.dataset.tab='standings';b.dataset.standingsTab='true';b.classList.toggle('active',active);b.setAttribute('aria-selected',active?'true':'false')}
  ...
  if(active&&superLeagueOwned())syncSuperLeagueStandings();
  else if(active&&signature!==lastSignature){panel.innerHTML=content();lastSignature=signature;bindPhoto()}
